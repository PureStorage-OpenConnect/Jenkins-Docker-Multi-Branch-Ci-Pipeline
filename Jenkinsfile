...
def StartContainer() {
    retry (3) {
        ...
        powershell "If (\$((docker logs ${CONTAINER_NAME} | select-string \"port is already allocated\" | select-string client).Length) -gt 0) { throw \"Error encountered when starting container\" }"    
    }
}
